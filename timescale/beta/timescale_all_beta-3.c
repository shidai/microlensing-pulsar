...
double g1 (double *k, size_t dim, void *params)
{
    ...
    delta_func=(1.0/(sqrt(2.0*3.1415926)*0.01))*exp(-0.5*(log10((re/v)/(24.0*60.0*60.0))-t)*(log10((re/v)/(24.0*60.0*60.0))-t)/(0.01*0.01));
        ...
        F=2.77*pow(10.0,-8.0)*pow(k[0],-6.0)*k[0]*k[0]*cos(b)*pnum*exp(-rds)*p_disk*(Deff)*(sqrt(M)*mfunc)*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0)*delta_func/4.3656;
    }
...
double g2 (double *k, size_t dim, void *params)
{
    ...
    delta_func=(1.0/(sqrt(2.0*3.1415926)*0.01))*exp(-0.5*(log10((re/v)/(24.0*60.0*60.0))-t)*(log10((re/v)/(24.0*60.0*60.0))-t)/(0.01*0.01));
        ...
        F=2.77*pow(10.0,-8.0)*pow(k[0],-6.0)*k[0]*k[0]*cos(b)*pnum*exp(-rds)*pnum2*exp(-rds2)*(Deff)*(sqrt(M)*mfunc)*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0)*delta_func/4.3656;
    }
...
{   
    ...
    delta_func=(1.0/(sqrt(2.0*3.1415926)*0.01))*exp(-0.5*(log10((re/v)/(24.0*60.0*60.0))-t)*(log10((re/v)/(24.0*60.0*60.0))-t)/(0.01*0.01));
        ...
        F=2.77*pow(10.0,-8.0)*pow(k[0],-6.0)*k[0]*k[0]*cos(b)*pnum*exp(-rds)*p_pulsar*(Deff)*(sqrt(M))*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0)*delta_func;
    }
...
double source (double *k, size_t dim, void *params)
{
    ...
    F=pow(k[0],-6.0)*k[0]*k[0]*cos(b)*pnum*exp(-rds);
...
int main (int argc, char *argv[])
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
        ...
        galaxy = (fabs(res)+fabs(res2)+fabs(resd)+fabs(res2d))/(fabs(res_s)+fabs(res_sd));
        ratio = ns+galaxy;
        printf ("%e %e\n", t, ratio);
        fflush(stdout);        
    }
