...
double g1 (double *k, size_t dim, void *params)
{
    ...
    vszfunc=(1.0/(sqrt(2.0*3.1415926)*sigma_vsz))*exp(-0.5*(vsz*vsz)/(sigma_vsz*sigma_vsz));
        ...
        F=2.77*pow(10.0,-8.0)*pow(k[0],-4.0)*k[0]*k[0]*cos(b)*pnum*exp(-rds)*p_disk*(Deff)*(sqrt(M)*mfunc)*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0)/4.3656;
    }
...
double g2 (double *k, size_t dim, void *params)
{
    ...
    // delta_func
        ...
        F=2.77*pow(10.0,-8.0)*pow(k[0],-4.0)*k[0]*k[0]*cos(b)*pnum*exp(-rds)*pnum2*exp(-rds2)*(Deff)*(sqrt(M)*mfunc)*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0)/4.3656;
    }
...
double g1d (double *k, size_t dim, void *params)
{
    ...
    {
        p_disk_s=pow(k[0],-4.0)*(p0/itad)*exp(-(rd-r0)/H)*((1.0-beta)*pow(cosh(zd/(itad*h1)),-2.0)+beta*exp(-fabs(zd)/(itad*h2)));  // :disk
    }
...
{   
    ...
    {
        p_disk=pow(k[0],-4.0)*(p0/ita)*exp(-(rdd-r0)/H)*((1.0-beta)*pow(cosh(zdd/(ita*h1)),-2.0)+beta*exp(-fabs(zdd)/(ita*h2)));  // :disk
    }
