...
double gd (double *k, size_t dim, void *params)
{
    ...
    {
        p_disk_s=(p0/itad)*(1.0/(k[0]*k[0]))*exp(-(rd-r0)/H)*((1.0-beta)*pow(cosh(zd/(itad*h1)),-2.0)+beta*exp(-fabs(zd)/(itad*h2)));  // :disk
    }
...
{   
    ...
    {
        p_disk=(p0/ita)*(1.0/(k[0]*k[0]))*exp(-(rdd-r0)/H)*((1.0-beta)*pow(cosh(zdd/(ita*h1)),-2.0)+beta*exp(-fabs(zdd)/(ita*h2)));  // :disk
    }
