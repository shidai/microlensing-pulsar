...
{   
    ...
    re=4.28*pow(10.0,8.0)*Deff*pow(M,0.5);   // km
        ...
        F=(re/v)*2.77*pow(10.0,-8.0)*pow(k[0],-6.0)*k[0]*k[0]*cos(b)*pnum*exp(-rds)*p_pulsar*(Deff)*(sqrt(M))*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0);
    }
